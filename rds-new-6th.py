...
DB_HOST = os.getenv("DB_HOST")  # Database host from environment variable
...
DB_NAME = os.getenv("DB_NAME")  # Database name from environment variable
...
def update_db_password(master_user, master_password, username, new_password):
...
def rotate_master_password(vault_client):
    ...
    try:
        update_db_password(master_user, master_password, master_user, new_master_password)
...
def rotate_user_passwords(vault_client, path, user_key, password_key, master_user, master_password):
    creds = get_credentials(vault_client, path)
    new_password = generate_new_password()
    
    try:
        update_db_password(master_user, master_password, creds[user_key], new_password)
...
def lambda_handler(event, context):
    ...
    rotate_user_passwords(vault_client, APP_CREDENTIALS_PATH, APP_READ_USER_KEY, APP_READ_USER_PASS_KEY, master_user, master_password)
    rotate_user_passwords(vault_client, APP_CREDENTIALS_PATH, APP_READWRITE_USER_KEY, APP_READWRITE_USER_PASS_KEY, master_user, master_password)
    rotate_user_passwords(vault_client, PIPELINE_CREDENTIALS_PATH, PIPELINE_READWRITE_USER_KEY, PIPELINE_READWRITE_USER_PASS_KEY, master_user, master_password)
